...
class TerrainController:
    ...
    def draw_terrain(self, canvas, start_x, start_y, iterations, x_end, y_end, offset_num, color):
        ...
        x_end = self.validate_numeric_input(x_end)
        y_end = self.validate_numeric_input(y_end)
        offset_num = self.validate_numeric_input(offset_num)
        if None in (start_x, start_y, iterations, x_end, y_end, offset_num):
            messagebox.showerror("Invalid Input", "Please enter valid numeric values.")
            return
        
        iterations = int(iterations)

        points = [(start_x, start_y), (x_end, y_end)]   
